    ...
    if menu_category.isdigit():
        ...
        if int(menu_category) in menu_items.keys():
            ...
            if menu_item_choice.isdigit():
                menu_item_choice = int(menu_item_choice)
                # 4. Check if the menu selection is in the menu items
                if menu_item_choice in menu_items:
                    selected_item = menu_items[menu_item_choice]
...
for item in order_list:
    ...
    print(f"{item_name}{' ' * name_spaces}| ${item_price:.2f}  | {item_quantity}")
